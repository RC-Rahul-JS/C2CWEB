...
const SignUp = () => {
    ...
    const [signupData, setSignupData] = useState({ fullName: '', gender: 'Male', phone: '', otp: '' });
    ...
    const verifyOTP = async () => {
        ...
        if (otp.length === 6 && /^\d{6}$/.test(otp)) {
            try {
                ...
                console.log(result)
                if (result.success) {
                    if (result.data.screen === 2) {
                        setInfoWithTimeout('Please complete your profile.');
                        setSignupData(prev => ({ ...prev, phone: phone }));
                        setMode('signup');
                        setStep(3);
                    } else {
                        setInfoWithTimeout('OTP verified successfully!');
                        login(result.data?.user || result.data, result.data.token);
                        setTimeout(() => {
                            const redirectPath = sessionStorage.getItem('redirectAfterLogin') || '/';
    ...
    const handleSignupSubmit = async (e) => {
        ...
        if (step === 1) {
            ...
            if (signupData.otp?.length === 6 && /^\d{6}$/.test(signupData.otp)) {
                ...
                const formattedPhone = signupData.phone;
                const result = await postapi('/c2c_app/verify-otp', {
                ...
                if (result.success) {
                    const res = await postapi('/c2c_app/register', {
                        name: signupData.fullName,
            ...
            const formattedPhone = formatPhoneForBackend(signupData.phone);
            const res = await postapi('/c2c_app/signup', {
                name: signupData.fullName,
                gender: signupData.gender,
            ...
            console.log(res)
            if (res.success) {
                setInfoWithTimeout('Profile Created Successfully!');
                login(res.data?.user || res.data, res.data.token);
                setTimeout(() => {
                    const redirectPath = sessionStorage.getItem('redirectAfterLogin') || '/';
                    sessionStorage.removeItem('redirectAfterLogin');
                ...
                setInfoWithTimeout(res.message || 'Registration failed');
            }
        }
    };
        ...
        <div className="min-h-screen w-full flex items-center justify-center bg-[#f0f4f8] font-sans p-4 mt-[40px]">
            ...
            <div className="flex w-full max-w-[980px] bg-white rounded-2xl shadow-xl overflow-visible min-h-[520px]">
                ...
                <div className="flex-1 p-6 md:p-10 bg-gradient-to-br from-[#4a69bd] to-[#3c54a6] text-white rounded-r-2xl md:rounded-l-none rounded-l-2xl flex flex-col justify-center relative">
                    ...
                    {mode === 'login' ? (
                        <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); step === 1 ? sendOTP() : verifyOTP(); }}>
                            {step === 1 ? (
                                <>
                                    ...
                                    <div className="flex items-center bg-white/15 border border-white/30 rounded-lg px-3">
                                        <span className="text-xs font-bold mr-2 text-white">+91</span>
                                        <input type="tel" maxLength={10} value={loginData.phone} onChange={(e) => setLoginData({ ...loginData, phone: e.target.value.replace(/\D/, '') })} placeholder="Enter Mobile" className="bg-transparent w-full py-2.5 text-sm outline-none text-white placeholder:text-white/50" />
                                    </div>
                                    <button type="submit" className="w-full bg-white text-[#4a69bd] py-3 rounded-lg font-bold shadow-lg hover:bg-gray-100 transition-colors mt-4">Continue</button>
                                </>
                            ) : (
                                <div className="flex flex-col items-center">
                                    <p className="text-white text-xs mb-4 opacity-80">Verify OTP sent to {loginData.phone}</p>
                                    <input type="text" maxLength={6} value={loginData.otp} onChange={(e) => setLoginData({ ...loginData, otp: e.target.value })} placeholder="••••••" className="w-full bg-white/15 border border-white/30 rounded-lg p-2.5 text-2xl tracking-[10px] text-center text-white outline-none placeholder:text-white/50 mb-2" autoFocus />
                                    <p className="text-white/50 text-xs mb-4">Resend OTP in {timer}s</p>

                                    <button type="submit" className="w-full bg-white text-[#4a69bd] py-3 rounded-lg font-bold shadow-lg hover:bg-gray-100 transition-colors mb-4">Verify OTP</button>
                                    <span onClick={() => setStep(1)} className="text-[#00E5FF] text-xs font-bold cursor-pointer underline">Change Number</span>
                                </div>
                            )}

                            <p className="text-xs text-center mt-6">Don't have an account? <span onClick={() => { setMode('signup'); setStep(1); }} className="underline cursor-pointer font-bold">Create Profile</span></p>
                        </form>
                    ) : (
                        <form className="space-y-3 no-scrollbar overflow-y-auto max-h-[380px]" onSubmit={handleSignupSubmit}>
                            ...
                            <input type="text" value={signupData.fullName} onChange={(e) => setSignupData({ ...signupData, fullName: e.target.value })} placeholder="Enter your name" className="w-full bg-white/15 border border-white/30 rounded-lg p-2.5 text-sm outline-none text-white placeholder:text-white/50" />

                            <label className="block text-xs font-semibold text-blue-100 mt-2">Gender</label>
                            <div className="flex justify-between gap-2 mb-2">
                                {['Male', 'Female', 'Other'].map((g) => (
                                    <button
                                        key={g} type="button"
                                        onClick={() => setSignupData({ ...signupData, gender: g })}
                                        className={`flex-1 py-2.5 rounded-lg border border-white/30 text-xs font-bold transition-all ${signupData.gender === g ? 'bg-white text-[#383981]' : 'bg-white/15 text-white'}`}
                                    >
                                        {g}
                                    </button>
                                ))}
                            ...
                            {step !== 3 && (
                                <>
                                    <label className="block text-xs font-semibold text-blue-100 mt-2">Mobile Number</label>
                                    <div className="flex items-center bg-white/15 border border-white/30 rounded-lg px-3">
                                        <span className="text-xs font-bold mr-2 text-white">+91</span>
                                        <input type="tel" maxLength={10} value={signupData.phone} onChange={(e) => setSignupData({ ...signupData, phone: e.target.value.replace(/\D/, '') })} placeholder="Number" className="bg-transparent w-full py-2.5 text-sm outline-none text-white placeholder:text-white/50" />
                                    </div>
                                </>
                            )}
                            ...
                            {step === 2 && (
                                <>
                                    <label className="block text-xs font-semibold text-blue-100 mt-2">Enter OTP</label>
                                    <input type="text" value={signupData.otp} onChange={(e) => setSignupData({ ...signupData, otp: e.target.value })} placeholder="6-digit OTP" className="w-full bg-white/15 border border-white/30 rounded-lg p-2.5 text-sm text-center tracking-[10px] outline-none text-white placeholder:text-white/50" />
                                </>
                            )}

                            <p className="text-[11px] mt-2"><input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} className="mr-2" /> I agree to the <a href="/terms" className="underline">Terms</a></p>
                            <button type="submit" disabled={!agree} className="w-full bg-white text-[#4a69bd] py-3 rounded-lg font-bold shadow-lg disabled:opacity-50 transition-all mt-4">{step === 1 ? 'Send OTP' : 'Finish Setup'}</button>
                            <p className="text-xs text-center mt-4">Already have account? <span onClick={() => { setMode('login'); setStep(1); }} className="underline cursor-pointer font-bold">Login here</span></p>
                        </form>
                    )}
                </div>
            </div>
